...
{
    // printf("\noffset is %ld. out=%d, in=%d\n", char_offset, buffer->out_offs, buffer->in_offs);
    size_t offset_left = char_offset;

    uint8_t index = 0 + buffer->out_offs;
    while (offset_left > 0)
    {
        size_t entry_size = buffer->entry[index].size;
        if (offset_left < entry_size)
        {
            break;
        }

        offset_left -= entry_size;
        index++;
